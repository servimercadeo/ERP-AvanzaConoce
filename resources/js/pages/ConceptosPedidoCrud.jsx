        ...
        try {
            ...
            setError(
                err?.response?.data?.errors?.nombre?.[0] ??
                    ...
                    "No se pudo guardar el concepto de pedido.",
            );
        } finally {
            setSaving(false);
        }
            ...
            <div style={S.modal} onClick={(e) => e.stopPropagation()}>
                <div style={S.modalHeaderGreen}>
                    <span style={S.modalTitleWhite}>
                        {editTarget ? "Editar Concepto de Pedido" : "Nuevo Concepto de Pedido"}
                    </span>
                ...
                <div style={S.modalBody}>
                    <div style={S.formGroup}>
                        ...
                        <input
                            ...
                            placeholder="Ej. DAÑO, ROBO, REPOSICION…"
                        />
...
/* ═══════════════════════════════════════════════════════════════════ */
export default function ConceptosPedidoCrud() {
    ...
    const { data: conceptos = [], isLoading } = useQuery({
        queryKey: ["conceptos-pedido-admin"],
    ...
    const filtered = useMemo(() => {
        ...
        return conceptos.filter((c) => norm(c.nombre ?? "").includes(q));
    }, [conceptos, debSearch]);
    ...
    const invalidate = () => {
        qc.invalidateQueries({ queryKey: ["conceptos-pedido-admin"] });
        qc.invalidateQueries({ queryKey: ["conceptos-pedido"] });
    };
    ...
    const handleSave = async (payload) => {
        if (editTarget) {
            await api.put(`/conceptos-pedido/${editTarget.id}`, payload);
            invalidate();
            showToast("Concepto de pedido actualizado.");
        } else {
            await api.post("/conceptos-pedido", payload);
            invalidate();
            showToast("Concepto de pedido creado.");
        }
    };

    const handleDelete = async (c) => {
        if (!confirm(`¿Eliminar el concepto de pedido "${c.nombre}"?`)) return;
        try {
            await api.delete(`/conceptos-pedido/${c.id}`);
            invalidate();
            showToast("Concepto de pedido eliminado.");
        } catch (err) {
            showToast(
                err?.response?.data?.message ??
                    "No se pudo eliminar el concepto de pedido.",
            );
        }
    };
        ...
        <div style={{ width: "100%" }}>
            ...
            <div className="stats-row">
                <div className="stat-card">
                    <div className="stat-num">{conceptos.length}</div>
                    <div className="stat-label">Total conceptos de pedido</div>
                </div>
            </div>
            ...
            <div style={S.tableWrap}>
                {isLoading ? (
                    ...
                    <div style={S.empty}>
                        ...
                        <p>No se encontraron conceptos de pedido.</p>
                    </div>
                ) : (
