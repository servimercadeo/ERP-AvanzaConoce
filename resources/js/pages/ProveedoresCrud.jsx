...
const NATURALEZAS = ["JURIDICA", "NATURAL", "GRAN CONTRIBUYENTE"];

const norm = (s = "") => s.toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g, "");

const badgeStyle = (naturaleza) => {
    if (naturaleza === "NATURAL") return { background: "#e0f2fe", color: "#0369a1" };
    if (naturaleza === "GRAN CONTRIBUYENTE") return { background: "#fef3c7", color: "#92400e" };
    return { background: "#eef1f5", color: "#5a6472" };
};

/* ─── Modal de creación / edición ──────────────────────────────────── */
function FormModal({ open, onClose, onSave, editTarget }) {
    const [nit, setNit] = useState("");
    const [naturaleza, setNaturaleza] = useState("JURIDICA");
    ...
    React.useEffect(() => {
        if (open) {
            ...
            setNaturaleza(editTarget?.naturaleza ?? "JURIDICA");
    ...
    const handleSave = async () => {
        if (!nit.trim() || !nombre.trim()) {
            setError("NIT y Nombre son requeridos");
            return;
        }
        setSaving(true);
        setError("");
        try {
            ...
            setError(
                err?.response?.data?.errors?.nit?.[0] ??
...
export default function ProveedoresCrud() {
    ...
    const filtered = useMemo(() => {
        ...
        return proveedores.filter(
            (p) => norm(p.nombre ?? "").includes(q) || norm(p.nit ?? "").includes(q),
        );
    }, [proveedores, debSearch]);
        ...
        <div style={{ width: "100%" }}>
            ...
            <div className="stats-row">
                <div className="stat-card">
                    <div className="stat-num">{proveedores.length}</div>
                ...
                {isLoading ? (
                    ...
                    <table className="data-table">
                        ...
                        <tbody>
                            {paginated.map((p) => (
                                <tr key={p.id}>
                                    <td style={{ fontWeight: 700, fontFamily: "monospace" }}>{p.nit}</td>
                                    <td>
                                        <span style={{
                                            fontSize: "0.75rem", fontWeight: 700, padding: "3px 10px",
                                            borderRadius: 20, whiteSpace: "nowrap", ...badgeStyle(p.naturaleza),
                                        }}>
                                            {p.naturaleza}
                                        </span>
                                    </td>
