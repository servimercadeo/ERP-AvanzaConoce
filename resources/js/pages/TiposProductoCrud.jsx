            ...
            <div style={S.modal} onClick={(e) => e.stopPropagation()}>
                <div style={S.modalHeaderGreen}>
                    <span style={S.modalTitleWhite}>
                        {editTarget ? "Editar Tipo de Producto" : "Nuevo Tipo de Producto"}
                    </span>
                ...
                <div style={S.modalBody}>
                    <div style={S.formGroup}>
                        ...
                        <input
                            ...
                            placeholder="Ej. EPP, ACTIVOS, HERRAMIENTA…"
                        />
...
export default function TiposProductoCrud() {
    ...
    const invalidate = () => {
        qc.invalidateQueries({ queryKey: ["tipos-producto-admin"] });
    ...
    const handleSave = async (payload) => {
        if (editTarget) {
            await api.put(`/tipos-producto/${editTarget.id}`, payload);
            invalidate();
            showToast("Tipo de producto actualizado.");
        } else {
    ...
    const handleDelete = async (t) => {
        if (!confirm(`¿Eliminar el tipo de producto "${t.nombre}"?`)) return;
        try {
            await api.delete(`/tipos-producto/${t.id}`);
            invalidate();
            showToast("Tipo de producto eliminado.");
        } catch (err) {
            showToast(
                err?.response?.data?.message ??
        ...
        <div style={{ width: "100%" }}>
            ...
            <div className="stats-row">
                <div className="stat-card">
                    <div className="stat-num">{tipos.length}</div>
                    <div className="stat-label">Total tipos de producto</div>
                </div>
            </div>
