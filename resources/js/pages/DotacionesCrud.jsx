...
const ESTADOS_ACTA  = ["Firmada", "Pendiente firma", "No aplica", "Por gestionar"];
const ESTADOS_CONT  = ["Activo", "Inactivo", "Retirado", "En licencia", "Suspendido"];
const POR_PAGINA    = 10;
...
const EMPTY_FORM = {
    sede: "", cedula: "", nombres: "", apellidos: "", cargo: "",
    tipo_contrato: "", proceso: "", fecha_ingreso: "",
    estado_contrato: "", empleador: "", proyecto: "",
    genero: "", ciudad: "",
    polo_masculino_talla: "", polo_masculino_cantidad: "",
    polo_femenino_talla: "", polo_femenino_cantidad: "",
    jean_masculino_talla: "", jean_masculino_cantidad: "",
    jean_femenino_talla: "", jean_femenino_cantidad: "",
    chaqueta_masculino_talla: "", chaqueta_masculino_cantidad: "",
    chaqueta_femenino_talla: "", chaqueta_femenino_cantidad: "",
    tenis_masculino_talla: "", tenis_masculino_cantidad: "",
    tenis_femenino_talla: "", tenis_femenino_cantidad: "",
    estado_acta: "", actas_sept: "",
    fecha_primera_renovacion_2024: "", fecha_segunda_renovacion_2024: "",
    fecha_tercera_renovacion_2024: "", fecha_primera_renovacion_2025: "",
    fecha_segunda_renovacion_2025: "",
    pedido_inicial: "", fecha_inicial: "",
    pedido_renovacion_1: "", fecha_renovacion_1: "",
    pedido_renovacion_2: "", fecha_renovacion_2: "",
    pedido_renovacion_3: "", fecha_renovacion_3: "",
    pedido_renovacion_4: "", fecha_renovacion_4: "",
    pedido_renovacion_5: "", fecha_renovacion_5: "",
};
...
function Field({ label, k, type = "text", opts, req, span, form, errors, onChange, disabled }) {
    ...
    const dis   = disabled ? { background: "var(--bg)", cursor: "default", color: "var(--text-muted)" } : {};
    return (
        <div style={style}>
            <label style={S.label}>{label}{req && !disabled ? " *" : ""}</label>
            {opts ? (
                <select
                    style={{ ...S.input, ...(errors[k] ? S.inputErr : {}), ...dis }}
                    value={form[k] ?? ""}
                    onChange={onChange(k)}
                    disabled={disabled}
                >
                ...
                <textarea
                    style={{ ...S.input, minHeight: 64, resize: disabled ? "none" : "vertical", ...dis }}
                    value={form[k] ?? ""}
                    onChange={onChange(k)}
                    disabled={disabled}
                />
...
function FilaDotacion({ label, prefix, tallasOpts, form, errors, onChange, disabled }) {
    ...
    return (
        <div style={{ marginBottom: 12 }}>
            ...
            <div style={{ ...S.grid4, marginTop: 10 }}>
                <Field label="Talla Masc." k={`${gM}_talla`}    opts={tallasOpts} form={form} errors={errors} onChange={onChange} disabled={isMaleDisabled} />
                <Field label="Cant. Masc." k={`${gM}_cantidad`} type="number"    form={form} errors={errors} onChange={onChange} disabled={isMaleDisabled} />
                <Field label="Talla Fem."  k={`${gF}_talla`}    opts={tallasOpts} form={form} errors={errors} onChange={onChange} disabled={isFemaleDisabled} />
                <Field label="Cant. Fem."  k={`${gF}_cantidad`} type="number"    form={form} errors={errors} onChange={onChange} disabled={isFemaleDisabled} />
            </div>
        </div>
    );
}
...
/* ─── Modal ────────────────────────────────────────────────── */
function Modal({ open, onClose, onSave, initial, title, options, empleados = [], readOnly = false }) {
    ...
    const handleEmployeeSelect = (emp) => {
        ...
        setForm((f) => {
            ...
            return {
                ...
                genero: nextGender,
                tipo_contrato: latestContrato.tipo_contrato || latestContrato.tipo_vinculacion || emp.tipo_vinculacion || f.tipo_contrato,
                estado_contrato: latestContrato.estado_contrato || emp.estado_empleado || f.estado_contrato,
                empleador: latestContrato.empleador || emp.empleador || f.empleador,
                proyecto: latestContrato.cliente_proyecto || emp.empresa?.nombre || f.proyecto,
                fecha_ingreso: latestContrato.fecha_ingreso ? dateOnly(latestContrato.fecha_ingreso) : f.fecha_ingreso,
    ...
    const handleSave = async () => {
        const e = validate();
        if (Object.keys(e).length) { setErrors(e); setActive("personal"); return; }
            ...
            <div style={{ ...S.modal, maxWidth: 960 }} onClick={(e) => e.stopPropagation()}>
                ...
                <div style={S.modalBody}>
                    ...
                    {activeTab === "personal" && (
                        <>
                            {!initial.id && (
                                <div style={{ marginBottom: 20, padding: "16px", background: "var(--bg)", borderRadius: "var(--radius-sm)", border: "1.5px dashed var(--primary)" }}>
                                    <label style={{ ...S.label, color: "var(--primary-dark)", fontSize: "0.82rem", display: "block", marginBottom: 8 }}>
                            ...
                            <div style={S.grid4}>
                                ...
                                <Field label="Nombres"   k="nombres"   req {...fp} span={2} />
                            </div>
                            <div style={{ ...S.grid4, marginTop: 14 }}>
                                <Field label="Sede"            k="sede"            opts={options.sedes}          {...fp} />
                                <Field label="Ciudad"          k="ciudad"                                        {...fp} />
                                <Field label="Cargo"           k="cargo"           opts={options.cargos}         {...fp} />
                                <Field label="Género"          k="genero"          opts={GENEROS}                {...fp} />
                            </div>
                            <div style={{ ...S.grid4, marginTop: 14 }}>
                                <Field label="Tipo Contrato"   k="tipo_contrato"                                 {...fp} />
                                <Field label="Proceso"         k="proceso"                                       {...fp} />
                                <Field label="Fecha Ingreso"   k="fecha_ingreso"   type="date"                   {...fp} />
                                <Field label="Estado Contrato" k="estado_contrato" opts={options.estados_contrato} {...fp} />
                            </div>
                            <div style={{ ...S.grid4, marginTop: 14 }}>
                                <Field label="Empleador" k="empleador" {...fp} span={2} />
                                <Field label="Proyecto"  k="proyecto"  {...fp} span={2} />
                            </div>
                        </>
                    )}
                    ...
                    {activeTab === "dotacion" && (
                        <>
                            <FilaDotacion label="Polo"     prefix="polo"     tallasOpts={TALLAS_ROPA}  {...fp} />
                            <FilaDotacion label="Jean"     prefix="jean"     tallasOpts={TALLAS_JEAN}  {...fp} />
                            <FilaDotacion label="Chaqueta" prefix="chaqueta" tallasOpts={TALLAS_ROPA}  {...fp} />
                            <FilaDotacion label="Tenis"    prefix="tenis"    tallasOpts={TALLAS_TENIS} {...fp} />
                        </>
                    )}
                    ...
                    {activeTab === "actas" && (
                        <>
                            <div style={S.grid4}>
                                <Field label="Estado Acta"  k="estado_acta"  opts={options.estados_acta} {...fp} />
                                <Field label="Actas Sept."  k="actas_sept"                               {...fp} />
                            </div>
                            <div style={S.sectionHeader2}>Renovaciones 2024</div>
                            <div style={{ ...S.grid3, marginTop: 10 }}>
                                <Field label="1ª Renovación 2024" k="fecha_primera_renovacion_2024" type="date" {...fp} />
                                <Field label="2ª Renovación 2024" k="fecha_segunda_renovacion_2024" type="date" {...fp} />
                                <Field label="3ª Renovación 2024" k="fecha_tercera_renovacion_2024" type="date" {...fp} />
                            </div>
                            <div style={S.sectionHeader2}>Renovaciones 2025</div>
                            <div style={{ ...S.grid3, marginTop: 10 }}>
                                <Field label="1ª Renovación 2025" k="fecha_primera_renovacion_2025" type="date" {...fp} />
                                <Field label="2ª Renovación 2025" k="fecha_segunda_renovacion_2025" type="date" {...fp} />
                            </div>
                        </>
                    )}
                    ...
                    {activeTab === "pedidos" && (
                        <>
                            ...
                            <div style={{ ...S.grid2, marginTop: 10 }}>
                                <Field label="No. Pedido Inicial" k="pedido_inicial" {...fp} />
                                <Field label="Fecha Inicial"      k="fecha_inicial"  type="date" {...fp} />
                            </div>
                            {[1, 2, 3, 4, 5].map((n) => (
                                <React.Fragment key={n}>
                                    <div style={S.sectionHeader2}>Renovación {n}</div>
                                    <div style={{ ...S.grid2, marginTop: 10 }}>
                                        <Field label={`No. Pedido Renov. ${n}`} k={`pedido_renovacion_${n}`} {...fp} />
                                        <Field label={`Fecha Renov. ${n}`}      k={`fecha_renovacion_${n}`}  {...fp} />
                                    </div>
                                </React.Fragment>
                            ))}
                        </>
            ...
            <div style={{ ...S.modal, maxWidth: 420 }} onClick={(e) => e.stopPropagation()}>
                ...
                <div style={{ padding: "28px 28px 20px" }}>
                    <p style={{ color: "var(--text)", lineHeight: 1.6 }}>
                        ¿Eliminar la dotación de <strong>{nombre}</strong>? Esta acción no se puede deshacer.
                    </p>
                </div>
...
export default function DotacionesCrud() {
    ...
    useEffect(() => { if (_qRegistros) setRegistros(_qRegistros); }, [_qRegistros]);
    ...
    const filtered = useMemo(() => {
        const palabras = debouncedSearch.toLowerCase().trim().split(/\s+/).filter(Boolean);
        return registros.filter((r) => {
            const hay = [r.cedula, r.nombres, r.apellidos, r.cargo, r.sede, r.proyecto, r.pedido_inicial]
                .join(" ").toLowerCase();
            const matchQ   = palabras.length === 0 || palabras.every((p) => hay.includes(p));
            const matchS   = filtroSede       === "Todas" || r.sede        === filtroSede;
            const matchA   = filtroEstadoActa === "Todos" || r.estado_acta === filtroEstadoActa;
            const matchG   = filtroGenero     === "Todos" || r.genero      === filtroGenero;
            const matchC   = filtroCargo      === "Todos" || r.cargo       === filtroCargo;
            const matchEC  = filtroEstadoCont === "Todos" || r.estado_contrato === filtroEstadoCont;
            return matchQ && matchS && matchA && matchG && matchC && matchEC;
        });
    }, [registros, debouncedSearch, filtroSede, filtroEstadoActa, filtroGenero, filtroCargo, filtroEstadoCont]);

    useEffect(() => setPagina(1), [debouncedSearch, filtroSede, filtroEstadoActa, filtroGenero, filtroCargo, filtroEstadoCont]);
    ...
    const firmadas      = registros.filter((r) => r.estado_acta === "Firmada").length;
    const proyectos     = new Set(registros.map((r) => r.proyecto).filter(Boolean)).size;
    const sedesActivas  = new Set(registros.map((r) => r.sede).filter(Boolean)).size;

    const showToast = (msg) => { setToast(msg); setTimeout(() => setToast(null), 3000); };

    const clearFilters = () => {
        setSearch(""); setFiltroSede("Todas"); setFiltroEstadoActa("Todos");
        setFiltroGenero("Todos"); setFiltroCargo("Todos"); setFiltroEstadoCont("Todos");
    };

    /* CRUD */
    const openCreate = () => { setEditTarget(null); setModalOpen(true); };
    const openEdit   = (r)  => { setEditTarget(r);  setModalOpen(true); };
    const openView   = (r)  => { setViewTarget(r);  setViewOpen(true); };
    const openDelete = (r)  => { setDeleteTarget(r); setDeleteOpen(true); };

    const handleSave = async (form) => {
        try {
            if (editTarget) {
                await api.put(`/dotaciones/${editTarget.id}`, form);
                const { data: fresh } = await api.get(`/dotaciones/${editTarget.id}`);
                setRegistros((prev) => prev.map((r) => (r.id === editTarget.id ? fresh : r)));
            ...
            console.error('[Dotacion] API error:', err?.response?.status, err?.response?.data);
    ...
    return (
        <div style={{ width: "100%" }}>
            ...
            <div className="stats-row">
                ...
                <div className="stat-card">
                    <div className="stat-num" style={{ color: "#27ae60" }}>{firmadas}</div>
                    <div className="stat-label">Actas firmadas</div>
                </div>
                <div className="stat-card">
                    <div className="stat-num" style={{ color: "var(--accent)" }}>{sedesActivas}</div>
                    <div className="stat-label">Sedes con dotación</div>
                </div>
                <div className="stat-card">
                    <div className="stat-num" style={{ color: "var(--primary)" }}>{proyectos}</div>
            ...
            <div style={S.toolbar}>
                <div style={S.filters}>
                    ...
                    <PresetFiltersDropdown presets={[
                        { label: "Actas firmadas",      apply: () => { clearFilters(); setFiltroEstadoActa("Firmada"); } },
                        { label: "Pendiente firma",     apply: () => { clearFilters(); setFiltroEstadoActa("Pendiente firma"); } },
                        { label: "Género masculino",    apply: () => { clearFilters(); setFiltroGenero("Masculino"); } },
                        { label: "Género femenino",     apply: () => { clearFilters(); setFiltroGenero("Femenino"); } },
                        { label: "Contratos activos",   apply: () => { clearFilters(); setFiltroEstadoCont("Activo"); } },
                        { label: "Limpiar filtros",     apply: () => clearFilters(), clear: true },
                    ]} />
                </div>
                <button className="btn-primary" onClick={openCreate}>+ Nueva dotación</button>
            </div>

            {/* ── Tabla ── */}
            <div style={S.tableWrap}>
                {loading ? (
                    <div style={S.empty}><IconLoading size={32} /><p>Cargando registros…</p></div>
                ) : filtered.length === 0 ? (
                    <div style={S.empty}><IconEmptySearch size={44} /><p>No se encontraron registros con los filtros aplicados.</p></div>
                ) : null}
                                ...
                                <tr key={r.id}>
                                    <td>
                                        <div style={S.avatarCell}>
                                            <div style={S.avatar}>
                                                {(r.nombres ?? "?").charAt(0).toUpperCase()}
                                            </div>
                                    ...
                                    <td style={{ fontSize: "0.84rem" }}>{r.cargo ?? "—"}</td>
                                    <td>
                                        {r.sede ? (
                                            <span style={S.badge("#e8f8f5", "var(--primary-dark)")}>{r.sede}</span>
                                        ) : "—"}
                                    </td>
                                    <td style={{ fontSize: "0.84rem", color: "var(--text-muted)" }}>{r.proyecto ?? "—"}</td>
                                    <td>
                                        {r.genero ? (
                                            <span style={S.badge(r.genero === "Masculino" ? "#e8f0ff" : "#fce8f5", r.genero === "Masculino" ? "#1a4fa8" : "#8b267a")}>
                                                {r.genero}
                                            </span>
                                        ) : "—"}
                                    </td>
                                    <td>
                                        <span style={{ ...badgeActa(r.estado_acta), display: "inline-flex", alignItems: "center", gap: 5 }}>
                                            <span style={{ width: 6, height: 6, borderRadius: "50%", background: r.estado_acta === "Firmada" ? "#27ae60" : "#e5a020", display: "inline-block" }} />
                                            {r.estado_acta ?? "—"}
                                        </span>
                                    </td>
                                    <td style={{ fontFamily: "monospace", fontSize: "0.82rem" }}>{r.pedido_inicial ?? "—"}</td>
                                    <td>
                                        <div style={S.actions}>
                                            <button style={S.actionBtn("#e8f0ff", "#1a4fa8")} title="Ver" onClick={() => openView(r)}>
                                                <IconEye />
                                            </button>
                                            <button style={S.actionBtn("#e8f8f5", "var(--primary-dark)")} title="Editar" onClick={() => openEdit(r)}>
                                                <IconEdit />
                                            </button>
                                            <button
                                                style={S.actionBtn("#fce8e8", "#c0392b")}
                                                title="Eliminar"
            ...
            {!loading && filtered.length > 0 && (
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 14, flexWrap: "wrap", gap: 10 }}>
                    <span style={{ color: "var(--text-muted)", fontSize: "0.82rem" }}>
                        Página {pagina} · Mostrando {Math.min(POR_PAGINA, filtered.length - (pagina - 1) * POR_PAGINA)} de {filtered.length}
                        {filtered.length !== registros.length ? ` (filtrados de ${registros.length})` : " registros"}
                    </span>
            ...
            <DeleteModal
                ...
                nombre={deleteTarget ? `${deleteTarget.nombres ?? ""} ${deleteTarget.apellidos ?? ""}`.trim() : ""}
            />
                        ...
                        <div style={S.modalBody}>
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "16px 24px" }}>
                                <div style={S.formGroup}>
                                    ...
                                    <SearchableSelect
                                        value={filtroSede} onChange={setFiltroSede} defaultValue="Todas"
                                        options={options.sedes.map((s) => ({ label: s, value: s }))}
                                    />
                                </div>
                                <div style={S.formGroup}>
                                    <label style={S.label}>Estado Acta</label>
                                    <SearchableSelect
                                        value={filtroEstadoActa} onChange={setFiltroEstadoActa} defaultValue="Todos"
                                        options={options.estados_acta.map((s) => ({ label: s, value: s }))}
                                    />
                                </div>
                                <div style={S.formGroup}>
                                    <label style={S.label}>Género</label>
                                    <SearchableSelect
                                        value={filtroGenero} onChange={setFiltroGenero} defaultValue="Todos"
                                ...
                                <div style={S.formGroup}>
                                    ...
                                    <SearchableSelect
                                        value={filtroCargo} onChange={setFiltroCargo} defaultValue="Todos"
                                        options={options.cargos.map((c) => ({ label: c, value: c }))}
                                    />
                                </div>
                                <div style={S.formGroup}>
                                    <label style={S.label}>Estado Contrato</label>
                                    <SearchableSelect
                                        value={filtroEstadoCont} onChange={setFiltroEstadoCont} defaultValue="Todos"
                                        options={options.estados_contrato.map((s) => ({ label: s, value: s }))}
                                    />
                                </div>
                            </div>
                        </div>
...
const S = {
    ...
    sectionHeader:  { marginTop: 20, marginBottom: 4, padding: "8px 14px", background: "var(--primary)", color: "#fff", borderRadius: "var(--radius-sm)", fontSize: "0.82rem", fontWeight: 800, letterSpacing: "0.05em", textAlign: "center" },
    sectionHeader2: { marginTop: 20, marginBottom: 0, padding: "7px 12px", background: "#e8f8f5", color: "var(--primary-dark)", borderRadius: "var(--radius-sm)", fontSize: "0.8rem", fontWeight: 800, letterSpacing: "0.04em" },
