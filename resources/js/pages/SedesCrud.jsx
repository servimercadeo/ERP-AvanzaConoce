...
function Modal({ open, onClose, onSave, initial, title, options, readOnly = false }) {
  ...
  useEffect(() => {
    if (open) {
      setForm({
        ...initial,
        departamento: initial.id_ciudad ? options.ciudades.find(c => c.id == initial.id_ciudad)?.id_departamento || '' : ''
      });
      setErrors({});
      setSaving(false);
    }
        ...
        <div style={S.modalBody}>
          <div style={S.grid3}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <Field label="Codigo Distribuidor" k="codigo_distribuidor" uppercase {...fp} />
              <Field label="Teléfonos" k="telefono" {...fp} />
              <Field label="Departamento" k="departamento" opts={departamentos.length > 0 ? departamentos : [{ value: '1', label: 'Sin Departamentos' }]} {...fp} />
              <Field label="Usuario Almacenista" k="id_almacenista_mac" opts={options.users} {...fp} />
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <Field label="Subcanal" k="sub_canal" opts={options.subcanales} {...fp} />
              <Field label="Estado" k="estado" opts={options.estados} req {...fp} />
              <Field label="Ciudad" k="id_ciudad" opts={ciudadesFiltradas.map(c => ({ value: c.id, label: c.nombre }))} req {...fp} />
              <Field label="Usuario Secretaria" k="id_secretaria_mac" opts={options.users} {...fp} />
            </div>

            {/* Right Column */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <Field label="Dirección" k="direccion" uppercase {...fp} />
              <Field label="Código C.I" k="codigo_instalador" uppercase {...fp} />
              <Field label="Tipo sede" k="tipo_sede" opts={options.tipos_sede} req {...fp} />
              <Field label="Sede padre" k="id_sede_padre" opts={options.sedes.map(s => ({ value: s.id, label: s.nombre }))} {...fp} />
              <Field label="Usuario Jefe Oficina" k="id_jefe_mac" opts={options.users} {...fp} />
            </div>
          </div>
        </div>
        ...
        <div style={S.modalFooter}>
          {readOnly ? (
            ...
            <>
              ...
              <button style={{ ...S.btnPrimaryGreen, opacity: saving ? 0.6 : 1 }} onClick={handleSave} disabled={saving}>
...
export default function SedesCrud() {
  ...
  const [options, setOptions] = useState({ users: [], ciudades: [], sedes: [], tipos_sede: [], estados: [], subcanales: [] });
  ...
  const filtered = useMemo(() => sedes.filter(s => {
    ...
    const matchQ = (s.nombre || '').toLowerCase().includes(q) || (s.codigo_distribuidor || '').toLowerCase().includes(q);
    const matchSede = filtroSede === 'Todas' || String(s.id) === filtroSede;
    const matchDepto = filtroDepto === 'Todos' || String(s.id_departamento) === filtroDepto;
    const matchTipo = filtroTipo === 'Todos' || s.tipo_sede === filtroTipo;
    const matchEstado = filtroEstado === 'Todas' || s.estado === filtroEstado;
    const matchCiudad = filtroCiudad === 'Todas' || String(s.id_ciudad) === filtroCiudad;
    return matchQ && matchSede && matchDepto && matchTipo && matchEstado && matchCiudad;
  }), [sedes, search, filtroSede, filtroDepto, filtroTipo, filtroEstado, filtroCiudad]);

  useEffect(() => { setPagina(1); }, [search, filtroSede, filtroDepto, filtroTipo, filtroEstado, filtroCiudad]);
  ...
  const stats = useMemo(() => ({
    total: sedes.length,
    principales: sedes.filter(s => s.tipo_sede === 'Principal').length,
  ...
  const handleSave = async form => {
    try {
      if (editTarget) {
        ...
        setSedes(prev => prev.map(s => s.id === editTarget.id ? data : s));
  ...
  const departamentos = useMemo(() => {
    const deps = new Set(options.ciudades.map(c => c.id_departamento).filter(Boolean));
    return Array.from(deps).map(d => ({ value: String(d), label: `Departamento ${d}` }));
  }, [options.ciudades]);
  ...
  return (
    <div style={{ width: '100%' }}>
      ...
      <div className="stats-row">
        ...
        <div className="stat-card">
          <div className="stat-num" style={{ color: '#1a9b8c' }}>{stats.principales}</div>
          <div className="stat-label">Sedes Principales</div>
        </div>
        <div className="stat-card">
          <div className="stat-num" style={{ color: '#27ae60' }}>{stats.activas}</div>
      ...
      <div style={S.toolbar}>
        ...
        <button style={{ ...S.btnPrimaryGreen, background: '#27ae60' }} onClick={() => { setEditTarget(null); setModalOpen(true); }}>Nuevo</button>
      </div>
      ...
      <div style={S.tableWrap}>
        {loading ? (
          ...
          <table className="data-table">
            <thead>
              <tr>
                <th style={{ width: 40, textAlign: 'center' }}>item</th>
                <th>Ciudad</th>
                <th>Nombre Sede</th>
            ...
            <tbody>
              {paginated.map((s, i) => (
                <tr key={s.id}>
                  <td style={{ textAlign: 'center', color: 'var(--text-muted)' }}>{(pagina - 1) * POR_PAGINA + i + 1}</td>
                  <td>{s.ciudad_nombre || '—'}</td>
                  <td style={{ fontWeight: 700, color: 'var(--primary-dark)' }}>{s.nombre}</td>
                  <td style={{ fontSize: '0.85rem' }}>{s.direccion || '—'}</td>
                  <td style={{ fontFamily: 'monospace', color: 'var(--text-muted)' }}>{s.telefono || '—'}</td>
                  <td><span style={S.badge(s.tipo_sede === 'Principal' ? '#e8f0ff' : '#f5f5f5', s.tipo_sede === 'Principal' ? '#1a4fa8' : '#555')}>{s.tipo_sede}</span></td>
                  <td>{s.padre?.nombre || '—'}</td>
                  <td>
                    <span style={S.badge(s.estado === 'Activa' ? '#e0f7f4' : '#fce8e8', s.estado === 'Activa' ? '#0d6e5a' : '#a33')}>
                      {s.estado}
                    </span>
                  </td>
      ...
      {!loading && filtered.length > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 14, flexWrap: 'wrap', gap: 10 }}>
          <span style={{ color: 'var(--text-muted)', fontSize: '0.82rem' }}>
            Página {pagina} · Mostrando {Math.min(POR_PAGINA, filtered.length - (pagina - 1) * POR_PAGINA)} de {filtered.length}{filtered.length !== sedes.length ? ` (filtrados de ${sedes.length})` : ' sedes'}
          </span>
      ...
      {filterOpen && (
        <div style={S.overlay} onClick={() => setFilterOpen(false)}>
          <div style={{ ...S.modal, maxWidth: 500 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={S.modalBody}>
              <div style={S.formGroup}>
                ...
                <select style={S.input} value={filtroSede} onChange={e => setFiltroSede(e.target.value)}>
                  ...
                  {options.sedes.map(s => <option key={s.id} value={s.id}>{s.nombre}</option>)}
                </select>
              </div>
              <div style={{ ...S.formGroup, marginTop: 16 }}>
                <label style={S.label}>Departamento</label>
                <select style={S.input} value={filtroDepto} onChange={e => setFiltroDepto(e.target.value)}>
              ...
              <div style={{ ...S.formGroup, marginTop: 16 }}>
                ...
                <select style={S.input} value={filtroCiudad} onChange={e => setFiltroCiudad(e.target.value)}>
                  <option value="Todas">Todas</option>
                  {options.ciudades.map(c => <option key={c.id} value={c.id}>{c.nombre}</option>)}
                </select>
              </div>
              <div style={{ ...S.formGroup, marginTop: 16 }}>
                ...
                <select style={S.input} value={filtroTipo} onChange={e => setFiltroTipo(e.target.value)}>
                  <option value="Todos">Todos</option>
                  {options.tipos_sede.map(t => <option key={t} value={t}>{t}</option>)}
                </select>
              </div>
              <div style={{ ...S.formGroup, marginTop: 16 }}>
            ...
            <div style={S.modalFooter}>
              <button style={S.btnSecondary} onClick={clearFilters}>Limpiar</button>
              <button style={S.btnPrimaryGreen} onClick={() => setFilterOpen(false)}>Aplicar</button>
            </div>
          </div>
        </div>
      )}

      <Modal open={modalOpen} onClose={() => setModalOpen(false)} onSave={handleSave} initial={editTarget || EMPTY_FORM} title={editTarget ? 'Sedes [ Edita ]' : 'Sedes [ Crea ]'} options={options} />
      <Modal open={viewOpen} onClose={() => setViewOpen(false)} initial={viewTarget || EMPTY_FORM} title="Sedes [ Ver ]" options={options} readOnly />
...
const S = {
  ...
  modalHeaderGreen: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 24px', borderBottom: '1px solid var(--border)', flexShrink: 0, background: 'var(--primary)' },
  modalTitleWhite: { fontFamily: "'Poppins',sans-serif", fontWeight: 600, fontSize: '1.2rem', color: '#fff' },
  closeBtnWhite: { background: 'transparent', border: '1px solid rgba(255,255,255,0.6)', borderRadius: '50%', width: 26, height: 26, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.9rem', cursor: 'pointer', color: '#fff' },
  modalBody: { padding: '24px', overflowY: 'auto', overflowX: 'hidden', flex: 1 },
  modalFooter: { display: 'flex', justifyContent: 'flex-end', gap: 12, padding: '16px 24px', borderTop: '1.5px solid var(--border)', flexShrink: 0 },
  grid3: { display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 24 },
  formGroup: { display: 'flex', flexDirection: 'column', gap: 4, minWidth: 0 },
  label: { fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-dark)' },
  input: { width: '100%', boxSizing: 'border-box', padding: '6px 8px', border: '1px solid var(--border)', borderRadius: '4px', fontSize: '0.85rem', fontFamily: 'Nunito,sans-serif', color: 'var(--text)', background: 'var(--white)', outline: 'none' },
  inputErr: { borderColor: '#e74c3c' },
  err: { color: '#e74c3c', fontSize: '0.7rem', marginTop: 2 },
  btnPrimary: { padding: '8px 16px', background: 'var(--primary)', color: '#fff', border: 'none', borderRadius: '4px', fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer', fontFamily: 'Nunito,sans-serif' },
  btnPrimaryGreen: { padding: '8px 16px', background: '#5cb85c', color: '#fff', border: 'none', borderRadius: '4px', fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer', fontFamily: 'Nunito,sans-serif' },
  btnSecondary: { padding: '8px 16px', background: '#d9534f', color: '#fff', border: 'none', borderRadius: '4px', fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer', fontFamily: 'Nunito,sans-serif' },
