            ...
            <div style={S.modal} onClick={(e) => e.stopPropagation()}>
                <div style={S.modalHeaderGreen}>
                    <span style={S.modalTitleWhite}>
                        {editTarget ? "Editar Categoría del Producto" : "Nueva Categoría del Producto"}
                    </span>
                ...
                <div style={S.modalBody}>
                    {!editTarget && (
                        <p style={{ margin: 0, fontSize: "0.82rem", color: "var(--text-muted)", lineHeight: 1.5 }}>
                            Al crearla aparecerá automáticamente como un nuevo módulo dentro de Inventarios,
                            con su propio inventario por sede.
                        </p>
                    )}
                    <div style={S.formGroup}>
                        ...
                        <input
                            ...
                            placeholder="Ej. Repuestos, Papelería…"
                        />
...
export default function CategoriaProductoCrud() {
    ...
    const { data: categorias = [], isLoading } = useQuery({
        queryKey: ["categorias-producto-admin"],
    ...
    const invalidate = () => {
        qc.invalidateQueries({ queryKey: ["categorias-producto-admin"] });
        qc.invalidateQueries({ queryKey: ["categorias-producto"] });
    ...
    const handleSave = async (payload) => {
        if (editTarget) {
            await api.put(`/categorias-producto/${editTarget.id}`, payload);
            invalidate();
            showToast("Categoría actualizada.");
        } else {
            await api.post("/categorias-producto", payload);
            invalidate();
            showToast("Categoría creada. Ya tiene su módulo en Inventarios.");
        }
    };
    ...
    const handleDelete = async (c) => {
        ...
        try {
            await api.delete(`/categorias-producto/${c.id}`);
            invalidate();
                ...
                {isLoading ? (
                    ...
                    <table className="data-table">
                        <thead>
                            <tr>
                                <th>Nombre</th>
                                <th>Módulo de inventario</th>
                                <th style={{ textAlign: "center" }}>Acciones</th>
                            </tr>
                        </thead>
                        <tbody>
                            {paginated.map((c) => {
                                const esFijo = CATEGORIAS_CON_MODULO_PROPIO.includes(c.nombre);
                                return (
                                    <tr key={c.id}>
                                        ...
                                        <td>
                                            <span style={{
                                                ...
                                                background: esFijo ? "#f1f5f9" : "#e0f7f4",
                                                color: esFijo ? "#475569" : "#0d6e5a",
                                            }}>
                                                {esFijo ? "Ya existente" : "Generado automáticamente"}
                                            </span>
                                        </td>
                                        <td>
                                            <div style={S.actions}>
