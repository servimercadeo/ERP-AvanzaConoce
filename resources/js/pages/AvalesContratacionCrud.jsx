    ...
    const handleOpenModal = (mode, row) => {
        ...
        setForm({
            ...
            fecha_programacion_ingreso:
                row.fecha_programacion_ingreso ||
                c.fecha_programacion_ingreso ||
                "",
    ...
    const filteredData = useMemo(
        () =>
            data.filter((row) => {
                ...
                const matchTipo =
                    tipoFilter === "Todos" ||
                    String(row.tipo_vinculacion ?? "").toLowerCase() ===
                        tipoFilter.toLowerCase();
                const matchEstado =
                    estadoFilter === "Todos" ||
    ...
    return (
        ...
        >
            ...
            <div style={S.toolbar}>
                ...
                <button style={S.btnSecondary} onClick={handleSync}>
                    Actualizar
                </button>
            </div>
                    ...
                    <table style={S.table}>
                        ...
                        <tbody>
                            {paginatedData.map((row, index) => {
                                ...
                                return (
                                    <tr key={row.id} style={S.tr}>
                                        ...
                                        <td style={S.td}>
                                            {fmt(row.documento_identificacion)}
                                        </td>
                                        <td
                                            style={{ ...S.td, fontWeight: 700 }}
                                        >
                                            {fmt(row.nombre_completo)}
                                        </td>
                                        ...
                                        <td style={S.td}>
                                            {fmtDate(row.fecha_correccion)}
                                        </td>
                                        <td style={S.td}>
                                            <span
                                                style={S.badge(ec.bg, ec.color)}
                                            >
                                                {row.estado
                                                    ? row.estado
                                                          .charAt(0)
                                        ...
                                        >
                                            <input
                                                type="checkbox"
                                                checked={
                                                    row.alerta_enviada ?? false
                                                }
                                                onChange={() =>
                                                    handleAlertToggle(row)
                                                }
                                                disabled={
                                                    row.alerta_enviada ||
                                                    alertLoading === row.id
                            ...
                            {filteredData.length === 0 && (
                                <tr>
                                    <td colSpan="11" style={S.empty}>
                                        No hay avales de contratación
                                        registrados.
                                    </td>
                                </tr>
                            )}
            ...
            {filteredData.length > POR_PAGINA && (
                <div style={S.paginationWrap}>
                    <span style={S.pageInfo}>
                        ...
                        {Math.min(
                            POR_PAGINA,
                            filteredData.length - (pagina - 1) * POR_PAGINA,
                        )}{" "}
                        ...
                        <div style={S.modalBody}>
                            ...
                            <div style={{ ...S.grid3, marginBottom: 20 }}>
                                ...
                                <ReadField
                                    label="Fecha exp. doc."
                                    value={
                                        form.candidato?.fecha_expedicion ?? "—"
                                    }
                                />
                                <ReadField
                                    label="Nombre"
                            ...
                            <div style={S.grid3}>
                                ...
                                <ReadField
                                    ...
                                    value={
                                        form.salario_basico
                                            ? `$${Number(form.salario_basico).toLocaleString()}`
                                            : "—"
                                    }
                                />
                                <ReadField
                                    ...
                                    value={
                                        form.auxilio_transporte
                                            ? `$${Number(form.auxilio_transporte).toLocaleString()}`
                                            : "—"
                                    }
                                />
                                <ReadField
                                    ...
                                    value={
                                        form.otrosi_variable
                                            ? `$${Number(form.otrosi_variable).toLocaleString()}`
                                            : "—"
                                    }
                                />
                                <ReadField
                                    label="Aux. rodamiento"
                                    value={
                                        form.auxilio_rodamiento
                                            ? `$${Number(form.auxilio_rodamiento).toLocaleString()}`
                                            : "—"
                                    }
                                />
                                <ReadField
                                    label="Aux. comunicación"
                                    value={
                                        form.auxilio_comunicacion
                                            ? `$${Number(form.auxilio_comunicacion).toLocaleString()}`
                                            : "—"
                                    }
                                />
                                <ReadField
                                    ...
                                    value={
                                        form.auxilio_alimentacion
                                            ? `$${Number(form.auxilio_alimentacion).toLocaleString()}`
                                            : "—"
                                    }
                                />
                            </div>
