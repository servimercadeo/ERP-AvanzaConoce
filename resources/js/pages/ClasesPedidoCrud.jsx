        ...
        try {
            ...
            setError(
                err?.response?.data?.errors?.nombre?.[0] ??
                    ...
                    "No se pudo guardar la clase de pedido.",
            );
        } finally {
            setSaving(false);
        }
            ...
            <div style={S.modal} onClick={(e) => e.stopPropagation()}>
                <div style={S.modalHeaderGreen}>
                    <span style={S.modalTitleWhite}>
                        {editTarget ? "Editar Clase de Pedido" : "Nueva Clase de Pedido"}
                    </span>
                ...
                <div style={S.modalBody}>
                    <div style={S.formGroup}>
                        ...
                        <input
                            ...
                            placeholder="Ej. OPTIMIZACION, Pedido Interno…"
                        />
...
export default function ClasesPedidoCrud() {
    ...
    const filtered = useMemo(() => {
        ...
        if (!q) return clases;
        return clases.filter((c) => norm(c.nombre ?? "").includes(q));
    }, [clases, debSearch]);
    ...
    const invalidate = () => {
        qc.invalidateQueries({ queryKey: ["clases-pedido-admin"] });
        qc.invalidateQueries({ queryKey: ["clases-pedido"] });
    };
    ...
    const handleSave = async (payload) => {
        if (editTarget) {
            await api.put(`/clases-pedido/${editTarget.id}`, payload);
            invalidate();
            showToast("Clase de pedido actualizada.");
        } else {
            await api.post("/clases-pedido", payload);
            invalidate();
            showToast("Clase de pedido creada.");
        }
    };

    const handleDelete = async (c) => {
        if (!confirm(`¿Eliminar la clase de pedido "${c.nombre}"?`)) return;
        try {
            await api.delete(`/clases-pedido/${c.id}`);
            invalidate();
            showToast("Clase de pedido eliminada.");
        } catch (err) {
            showToast(
                err?.response?.data?.message ??
                    "No se pudo eliminar la clase de pedido.",
            );
        }
    };
        ...
        <div style={{ width: "100%" }}>
            ...
            <div className="stats-row">
                <div className="stat-card">
                    ...
                    <div className="stat-label">Total clases de pedido</div>
                </div>
            </div>
            ...
            <div style={S.tableWrap}>
                {isLoading ? (
                    ...
                    <div style={S.empty}>
                        ...
                        <p>No se encontraron clases de pedido.</p>
                    </div>
                ) : (
