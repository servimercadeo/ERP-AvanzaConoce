    ...
    <Layout>
      <div style={{ width: '100%', maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <div className="submod-content">
          ...
          {sub.archivos && sub.archivos.length > 0 && (
            <div className="archivos-section" style={{ marginTop: '32px', marginBottom: '24px' }}>
              <p className="section-title" style={{ fontSize: '1.1rem', marginBottom: '16px' }}>Archivos disponibles</p>
              <div className="mod-subgrid">
                {sub.archivos.map(archivo => (
                  <Link key={archivo.id} className="submod-card" to={`/module/${mod.id}/submodule/${sub.id}`}>
                    <span className="sc-icon">📄</span>
                    <span className="sc-label">{archivo.label}</span>
                    <span className="sc-desc">Acceder a los registros de {archivo.label.toLowerCase()}</span>
                  </Link>
                ))}
              </div>
            </div>
