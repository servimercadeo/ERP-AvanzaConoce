...
const IcoMoon = () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>
const IcoReset = () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 .49-4.95"/></svg>
...
const IcoCheck = () => <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3.5"><polyline points="20 6 9 17 4 12"/></svg>
const IcoMagic = () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
    ...
    <div className="tpb-toggle-group">
      {options.map(o => (
        <button
          ...
          title={o.label}
        >
          {o.icon && <span className="tpb-toggle-icon">{o.icon}</span>}
          <span>{o.label}</span>
        </button>
      ))}
    </div>
...
export default function ThemePickerButton() {
  ...
  return (
    <>
      {open && <div className="theme-picker-overlay" onClick={() => setOpen(false)} />}
        ...
        <div className="theme-picker-panel tpb-panel tpb-panel--extended">
          ...
          <div className="tpb-header">
            ...
            <div className="tpb-header-text">
              ...
              <p className="tpb-header-sub">Configura tu espacio de trabajo</p>
            </div>
            {!user && <span className="tpb-local-badge">local</span>}
            <button className="tpb-close-btn" onClick={() => setOpen(false)}>
              <IcoClose />
            </button>
          </div>
          ...
          <div className="tpb-scroll-area">
            ...
            <Section title="Paleta de Colores">
              ...
              <p className="tpb-active-label">{THEMES[prefs.theme]?.label}</p>
            </Section>
            ...
            <Section title="Efectos Visuales">
              <div className="tpb-grid-2">
                <div className="tpb-toggle-group">
                  <button className={`tpb-toggle-btn${!prefs.dark ? ' tpb-toggle-btn--active' : ''}`} onClick={() => set('dark', false)}>
                    <IcoSun /> Claro
                  </button>
                  <button className={`tpb-toggle-btn${prefs.dark ? ' tpb-toggle-btn--active' : ''}`} onClick={() => set('dark', true)}>
                    <IcoMoon /> Oscuro
                  </button>
                </div>
              </div>
            ...
            <Section title="Textos y Tipografía">
              ...
              />
              <p className="tpb-sub-title" style={{ marginTop: '10px' }}>Tamaño de interfaz</p>
              <ToggleGroup
            ...
            <Section title="Estructura y Sombras">
              <p className="tpb-sub-title">Bordes de componentes</p>
              <ToggleGroup
                value={prefs.radius}
                onChange={v => set('radius', v)}
                options={[
                  { value: 'sharp',   label: 'Recto',    icon: <span className="tpb-radius-preview tpb-radius-sharp" /> },
                  { value: 'default', label: 'Suave',    icon: <span className="tpb-radius-preview tpb-radius-default" /> },
                  { value: 'pill',    label: 'Cápsula',  icon: <span className="tpb-radius-preview tpb-radius-pill" /> },
                ]}
              />
              <p className="tpb-sub-title" style={{ marginTop: '10px' }}>Intensidad de sombras</p>
              <ToggleGroup
                value={prefs.shadows}
            ...
            <Section title="Navegación">
              ...
              />
              <p className="tpb-sub-title" style={{ marginTop: '10px' }}>Barra superior (Header)</p>
              <ToggleGroup
                value={prefs.navbar}
          ...
          <div className="tpb-footer tpb-footer--sticky">
            <button className="tpb-reset-btn" onClick={handleReset} disabled={resetting}>
              <IcoReset />
